          ...
          <Carousel className="w-full">
            <CarouselContent>
              {skillsWithFavicons.map((skill, index) => (
                <CarouselItem key={skill.name} className="md:basis-1/3 lg:basis-1/4">
                  <Card className="p-4 bg-card/80 border-border text-center shadow-sm flex flex-col items-center gap-2">
                    <img 
                      src={skill.favicon} 
                      alt={skill.name}
                      className="w-8 h-8"
